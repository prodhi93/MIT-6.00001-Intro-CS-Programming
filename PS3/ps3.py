...
def get_word_score(word, n):
    ...
    for letter in word:
        sum_pts += SCRABBLE_LETTER_VALUES[letter]
        
    if (7*wordlen - 3*(n-wordlen)) > 1:
        score = sum_pts * (7*wordlen - 3*(n-wordlen))
    else:
        score = sum_pts
...
def update_hand(hand, word):
    ...
    word = word.lower()
    freqs = get_frequency_dict(word)
    new_hand = {}
    
    for letter in hand:
        if letter in word:
            new_hand[letter] = hand[letter] - freqs.get(letter,0)
        else:
...
def is_valid_word(word, hand, word_list):
    ...
    if "*" in word:
        ...
        for vowel in VOWELS:
            ...
            word_options.append(poss_word)
        valid = any(poss_word in word_list for poss_word in word_options)
    ...
    if valid == True:
        for letter in word:
            if freqs[letter] > hand.get(letter,0):
                valid = False
                break
    ...
    # As long as there are still letters left in the hand:
        ...
        if word == "!!": # If the input is two exclamation points:
            ...
            if is_valid_word(word, hand, word_list): # If the word is valid:
                ...
                quotes = '"'
                print(quotes+word+quotes,' earned ', word_score, 'points. Total: ',total_score, 'points')
                print()
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(total_hands):
        ...
        while replay >= 0:
            ...
            display_hand(hand)
            print()
            want_sub = input("Would you like to substitute a letter? ")
    
            if want_sub.lower() == "yes":
                letter = input("Which letter would you like to replace? ")
                sub_hand = substitute_hand(hand, letter)
                #play_hand(subhand, word_list)
                hand_score = play_hand(sub_hand, word_list)
            elif want_sub.lower() == "no":
